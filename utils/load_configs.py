...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='CanParl',
                        choices=['wikipedia', 'reddit', 'mooc', 'lastfm', 'Synthetic','enron', 'SocialEvo', 'uci', 'Flights', 'CanParl', 'USLegis', 'UNtrade', 'UNvote', 'Contacts'])
    ...
    parser.add_argument('--before', action='store_true', default=False, help='whether to load the best configurations')

    try:
        args = parser.parse_args()
        args.device = f'cuda' if torch.cuda.is_available() and args.gpu >= 0 else 'cpu'
    except:
        parser.print_help()
        sys.exit()
...
def get_node_classification_args():
    ...
    parser.add_argument('--test_interval_epochs', type=int, default=10, help='how many epochs to perform testing once')
    parser.add_argument('--load_best_configs', action='store_true', default=False, help='whether to load the best configurations')
    parser.add_argument('--before', action='store_true', default=True, help='whether to load the best configurations')
