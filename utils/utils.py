...
class NeighborSampler:
    ...
    def find_neighbors_before(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False,dst_node_ids = None):
        """
    ...
    def find_neighbors_current(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False,dst_node_ids = None ):
        """
        extracts all the interactions happening before interact_time (less than interact_time) for node_id in the overall interaction graph
        the returned interactions are sorted by time.
        :param node_id: int, node id
        ...
        neighbor_ids = self.nodes_neighbor_ids[node_id][:i]
  
        index_to_remove = np.where(neighbor_ids == dst_node_ids)[0]
        edges_ids = self.nodes_edge_ids[node_id][:i]
        nodes_neighbor_times =self.nodes_neighbor_times[node_id][:i]

        if len(index_to_remove) > 0:

            neighbor_ids = np.delete(neighbor_ids, index_to_remove[0])
            edges_ids = np.delete(edges_ids,index_to_remove[0])
            nodes_neighbor_times = np.delete(nodes_neighbor_times, index_to_remove[0])
     




        return neighbor_ids, edges_ids,nodes_neighbor_times, None
    ...
    def get_all_first_hop_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray,before = True,dst_node_ids  = None):
        """
        ...
        """
        if before :
            finder = self.find_neighbors_before

        else:
            finder = self.find_neighbors_current
        ...
        # get the temporal neighbors at the first hop
        for idx, (node_id, node_interact_time,dst_id) in enumerate(zip(node_ids, node_interact_times,dst_node_ids)):
            # find neighbors that interacted with node_id before time node_interact_time
            node_neighbor_ids, node_edge_ids, node_neighbor_times, _ = finder(node_id=node_id, interact_time=node_interact_time,
                                                                                                  return_sampled_probabilities=False,dst_node_ids = dst_id)
