...
def evaluate_model_link_prediction_snapshot(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluation_data_loaders,
                                   evaluate_neg_edge_sampler: NegativeEdgeSampler, evaluation_datas: Data, loss_func: nn.Module,
                                   num_neighbors: int = 20, time_gap: int = 2000, before = True):
    """
    evaluate models on the link prediction task
    :param model_name: str, name of the model
    :param model: nn.Module, the model to be evaluated
    ...
    snapshots = evaluation_data_loaders.keys()
    ...
    with torch.no_grad():
        for ts in snapshots :
            evaluate_idx_data_loader = evaluation_data_loaders[ts]
            evaluate_data = evaluation_datas[ts]['edges']
            edge_features = evaluation_datas[ts]['edge_features']
            node_features = evaluation_datas[ts]['node_features']
            ...
            for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
                ...
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                                                                        dst_node_ids=batch_dst_node_ids,
                                                                        node_interact_times=batch_node_interact_times,before=before)
                ...
                evaluate_idx_data_loader_tqdm.set_description(f'evaluate for the {batch_idx + 1}-th batch, at timestamp : {ts},  evaluate loss: {np.mean(evaluate_losses)}')
